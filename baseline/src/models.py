...
def get_model(model_name, num_features, num_classes, edge_index, x, args, mask=None, train_mask=None):
...
class GNN(torch.nn.Module):
    def __init__(
        self, num_features, num_classes, hidden_dim, num_layers=2, dropout=0, conv_type="gat", jumping_knowledge=False,
    ):
...
class PaGNN(torch.nn.Module):
    def __init__(
        self, num_features, num_classes, hidden_dim, num_layers=2, dropout=0, mask=None, edge_index=None, train_mask=None,
    ):
        ...
        self.convs = ModuleList([PaGNNConv(num_features, hidden_dim, mask, edge_index, train_mask)])
...
class GCNmfEncoder(torch.nn.Module):
    def __init__(
        self, num_features, out_channels, hidden_dim, x, edge_index, dropout=0, n_components=5,
    ):
        super(GCNmfEncoder, self).__init__()
        self.gc1 = GCNmfConv(
            in_features=num_features,
            out_features=hidden_dim,
            x=x,
        ...
        self.gc2 = GCNConv(hidden_dim, out_channels, dropout)
        self.dropout = dropout
        self.reset_parameters()
