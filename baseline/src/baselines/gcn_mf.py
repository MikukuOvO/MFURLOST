...
class GCNmfConv(nn.Module):
    def __init__(self, in_features, out_features, x, edge_index, n_components, dropout, bias=True):
        ...
        _, weights = gcn_norm(edge_index, num_nodes=x.shape[0], add_self_loops=False)
        # self.adj = torch.sparse_coo_tensor(edge_index, weights, (edge_index.max().item() + 1, edge_index.max().item() + 1)).to(self.device)
        self.adj = torch.sparse_coo_tensor(edge_index, weights, (x.shape[0], x.shape[0])).to(self.device)
    ...
    def forward(self, x, edge_index):
        ...
        ### Modify
        _, weights = gcn_norm(edge_index, num_nodes=x.shape[0], add_self_loops=False)
        # new_adj = torch.sparse_coo_tensor(edge_index, weights, (edge_index.max().item() + 1, edge_index.max().item() + 1)).to(self.device)
        new_adj = torch.sparse_coo_tensor(edge_index, weights, (x.shape[0], x.shape[0])).to(self.device)
        # Compute element-wise square of adjacency matrix
        new_adj2 = torch.mul(new_adj, new_adj)

        conv_x = []
        conv_covs = []
        for component_x in transform_x:
            conv_x.append(torch.spmm(new_adj, component_x))
        for component_covs in transform_covs:
            conv_covs.append(torch.spmm(new_adj2, component_covs))
