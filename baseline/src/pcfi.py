...
class PCFI(torch.nn.Module):
    ...
    def compute_f_n2d(self, edge_index, feature_mask, mask_type, feat_dim: OptTensor = None):
        ...
        if mask_type == 'structural':
            len_v_0tod_list = []
            f_n2d = torch.zeros(nv, dtype = torch.int).to(edge_index.device)
            ...
            f_n2d = torch.zeros(feat_dim, nv).to(edge_index.device)
    ...
    def compute_edge_weight_c(self, edge_index, f_n2d, n_nodes):
        ...
        d_row = f_n2d[row].to(edge_index.device)
        d_col = f_n2d[col].to(edge_index.device)
